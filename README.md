...
毎週日曜22:30 JSTに「Model evaluation」が蓄積結果を時系列順に80%の学習用、20%の未使用検証用へ分け、現在の重みと改善候補を比較します。未来のレース結果を過去の予想へ混ぜない方式です。

- 結果確定済み200レース未満：データ不足として現行モデルを維持
- 200レース以上：240通りの重み候補を学習区間で比較
- 学習区間では回収率70%、3連単的中率20%、1着精度10%の比重で候補を比較
- 未使用検証区間でも回収率が10ポイント以上改善し、3連単的中率の低下が2ポイント以内の候補だけ推薦
- 推薦重みは自動適用せず、レポートを保存して人が確認

```bash
python -m boatrace_ai --db data/boatrace.sqlite3 --evaluate-model
```

レポートは `artifacts/learning-report.json` とSQLiteへ保存され、次回の画面用 `latest.json` の `model_evaluation` に含まれます。回収率を最優先にしますが、短期間の高配当1件へ過学習しないよう回収率スコアに上限を設け、時系列で分離した未使用期間と最低的中率条件でも確認します。
