...
- 未使用検証区間で1着精度が2ポイント以上改善し、3連単的中率が悪化しない候補だけ推薦
...
レポートは `artifacts/learning-report.json` とSQLiteへ保存され、次回の画面用 `latest.json` の `model_evaluation` に含まれます。短期間の回収率だけを追って過学習しないよう、候補選択は1着精度と3連単的中率を中心に行います。
